    ...
    def inference_with_vad(self, input, input_len=None, **cfg):
        ...
        for i in range(len(res)):
            ...
            for j, _ in enumerate(range(0, n)):
                ...
                #################
                # if model.__class__.__name__ == "FunASRNano":
                #     conv_results = []
                ...
                #         for part in result["timestamps"]:
                #             word = part["token"]
                #             if word not in ['。', '，', '！', '？', '、', '；', '：', ',', '.', '?', '!']:
                #                 text.append(word)
                #                 start_ms = int(part["start_time"] * 1000)
                #                 end_ms = int(part["end_time"] * 1000)
                #                 timestamp.append([start_ms, end_ms])

                #         new_result["text"] = ' '.join(text)
                #         new_result["timestamp"] = timestamp
                #         conv_results.append(new_result)
                    ...
                    if k.startswith("timestamp"):
                        ...
                        for t in restored_data[j][k]:
                            if isinstance(t, (list, tuple)):
                                ...
                                # FunASRNano 的时间戳单位是秒，vadsegments 是毫秒，需转换
                                if "start_time" in t:
                                    t["start_time"] = t["start_time"] + vadsegments[j][0] / 1000.0
                                if "end_time" in t:
                                    t["end_time"] = t["end_time"] + vadsegments[j][0] / 1000.0
