"""
FunASR-Nano ASR 模块（多语言，字级时间戳转句级）
...
SYS_PATH    = os.path.join(_PROJECT_ROOT, "models/FunAudioLLM/Fun-ASR-Nano-2512")
VAD_MODEL   = "iic/speech_fsmn_vad_zh-cn-16k-common-pytorch"


def load_model(language, hotwords, spk_num, logger, **kwargs):
    # 注册自定义模型，否则报 "FunASRNano is not registered"
    if SYS_PATH not in sys.path:
        sys.path.append(SYS_PATH)

    from funasr import AutoModel

    logger.info("加载 FunASRNano 模型...")
    model = AutoModel(
        model=MODEL_PATH,
        vad_model=VAD_MODEL,
...
def recognize(model, generate_kwargs, audio_path, logger):
    ...
    logger.debug(f"FunASRNano 原始结果:\n{res}\n")
